...
* **🚀 Hybrid Classifier**: Kết hợp Regex tiếng Việt tốc độ cao (nhãn cứng) và Gemini 2.5 Flash (nhãn phức tạp).
* **⚡ Incremental Delta**: So khớp khóa chính `ActivityId` để chỉ phân loại dòng mới, tiết kiệm tối đa chi phí gọi API.
* **🔒 Zero Row-Shifting**: Đồng bộ an toàn bằng ID thay vì số thứ tự dòng vật lý, chống lệch cột khi người dùng sửa đổi cấu trúc dòng Excel trên SharePoint.
* **🛠️ Checkpoint tự phục hồi**: Chia nhỏ batch và lưu tiến trình liên tục, tự động chạy tiếp khi xảy ra lỗi kết nối.
